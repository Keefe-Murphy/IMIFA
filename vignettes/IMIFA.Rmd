...
Let's examine the posterior distribution of $G$ and the posterior distribution of $Q_g$ for each of the `r resIMIFA$GQ.results$G` clusters. The third plot below, depicting the trace of the numbers of active and non-empty clusters, allows us to examine mixing of the chain with respect to $G$.
...
The error between the empirical and estimated covariance matrices (by cluster and averaged across clusters) can sometimes provide a useful indicator of the validity of a clustering solution, and can be visualised as follows:
```{r}
...
Murphy, K., Gormley, I. C. and Viroli, C. (2017). Infinite Mixtures of Infinite Factor Analysers: Nonparametric Model-Based Clustering via Latent Gaussian Models. _To appear_. Pre-print available at [arXiv:1701.07010](https://arxiv.org/abs/1701.07010).
