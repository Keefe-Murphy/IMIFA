...
Finally, let's run the flagship IMIFA model, on which all subsequent demonstrations and results will be based, for a greater number of iterations, accepting all argument defaults. Note that the `verbose` argument, which defaults to `TRUE` will ordinarily print a progress bar to the console window.

```{r, eval=FALSE}
simIMIFA <- mcmc_IMIFA(olive, method="IMIFA", n.iters=50000)
```

```{r, progess='hide', results='hide', include=FALSE}
simIMIFA <- mcmc_IMIFA(olive, method="IMIFA", n.iters=50000, rho=0.7)
```
...
resIMIFA <- try(get_IMIFA_results(simIMIFA, zlabels=area, G=4), silent=TRUE)
while(inherits(resIMIFA, "try-error")) {
  simIMIFA <- mcmc_IMIFA(olive, method="IMIFA", n.iters=50000, rho=0.7)
  resIMIFA <- try(get_IMIFA_results(simIMIFA, zlabels=area, G=4), silent=TRUE)
}
```
...
Let's examine the posterior distribution of $Q_g$ for each of the `r resIMIFA$GQ.results$G` groups. In this case, `g=1` would mean to instead examine the posterior distribution of $G$).
...
```
To examine the trace plots for the factor score parameters, where `mat=TRUE` means, in this case, to all plot all observations simultaneously (for a given factor, say the 1st), we would type

```{r, eval=FALSE}
plot(resIMIFA, plot.meth="trace", param="scores", mat=TRUE, fac=1)
```

Let's instead plot all factors simultaneously for a given observation (say the 2nd), by specifying `by.fac=FALSE`:
```{r}
plot(resIMIFA, plot.meth="trace", param="scores", mat=TRUE, ind=2, by.fac=FALSE)
```
