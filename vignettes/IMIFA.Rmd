...
```{r, eval=FALSE}
simOMIFA <- mcmc_IMIFA(olive, method="OMIFA", range.G=10, nu=3, alpha=0.8,
...
Let's examine clustering performance against the known group labels.

```{r, fig.show='hold'}
plot(resIMIFA, plot.meth="zlabels", zlabels=area)
```

To examine the mean parameters of the 1st cluster, where `all` below refers to trace, density, posterior mean, and ACF plots, and `mat=TRUE` means all variables simultaneously, we can type

```{r, fig.show='hold'}
plot(resIMIFA, plot.meth="all", param="means", mat=TRUE, g=1)
```
...
The error between the empirical and estimated covariance matrices (by group and averaged across groups) can be visualised as follows
```{r, fig.show='hold'}
plot(resIMIFA, plot.meth="errors")
```

Finally, inference can be conducted on the Dirichlet process concentration parameter $\alpha$ (assuming `alpha.step` was set to `"fixed"` when `mcmc_IMIFA` was initially run using the IMFA/IMIFA methods).
```{r, fig.show='hold'}
...
Murphy, K., Gormley, I.C. and Viroli, C. (2017) Infinite Mixtures of Infinite Factor Analysers: Nonparametric Model-Based Clustering via Latent Gaussian Models, [https://arxiv.org/abs/1701.07010](_to appear_)
