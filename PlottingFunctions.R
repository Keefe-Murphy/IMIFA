...
plot.IMIFA  <- function(results=NULL, plot.meth=c("all", "correlation", "density", "posterior", "Q", "trace"), 
  ...
  if(plot.meth == "all")  {
    ...
    par(oma=c(0, 0, 2, 0), mai=c(0.7, 0.7, 0.5, 0.2), mgp=c(2, 1, 0))
    cex.t   <- 0.8
    all.ind <- T
  } else {
  ...
  if(m.sw["tra.sw"]) {
    ...
    if(var == "loadings") {
      ...
      if(mat) {
        ...
        title(main=list(paste0("Trace", ifelse(all.ind, ":\n", ":\nLoadings - "), rownames(plot.x)[ind[1]], " Variable"), cex=cex.t))
      } else {
    ...
    if(!ind.x)             ind <- x.ind
    if(all.ind) title(paste0(toupper(substr(var, 1, 1)),
                      substr(var, 2, nchar(var))), outer=T)
  }
