...
# Gibbs Sampler Function
  gibbs.IMIFA        <- function(Q, data, iters, N, P, G, mu.zero, rho, sigma.l, alpha.step, mu, sw, 
                                 ...
                                 nu, prop, d.hyper, beta.d1, beta.d2, adapt.at, epsilon, learn.d, ...) {
  ...
  # Define & initialise variables
    ...
    phi              <- lapply(Ts, function(t) sim.phi.p(Q=Q, P=P, nu=nu))
  ...
  # Iterate
    ...
    # Cluster Labels
      ...
      Q0             <- Qgs  > 0
      Q1             <- Qgs == 1
      z.log          <- capture.output({ z.res <- try(sim.z(data=data, mu=mu[,Gs, drop=FALSE], sigma=sigma, Gseq=Gs, N=N, pi.prop=pi.prop[Gs], log.slice.ind=log.slice.ind, Q0=Q0[Gs]), silent=TRUE) })
      z.err          <- inherits(z.res, "try-error")
      if(z.err) {
        sigma        <- lapply(sigma, make.positive.definite)
        z.res        <- sim.z(data=data, mu=mu[,Gs, drop=FALSE], sigma=sigma, Gseq=Gs, N=N, pi.prop=pi.prop[Gs], log.slice.ind=log.slice.ind, Q0=Q0[Gs])
      }
      z              <- z.res$z
      nn             <- tabulate(z, nbins=trunc.G)
      nn0            <- nn   > 0
    ...
    # Scores & Loadings
      ...
      if(!any(Q0))    {
        ...
        lmat[Gs]     <- lapply(Gs, function(g) base::matrix(, nr=P, nc=0))
      } else {
        eta.tmp      <- lapply(Gs, function(g) if(all(nn0[g], Q0[g])) sim.score(N=nn[g], lmat=lmat[[g]], Q=Qs[g], Q1=Q1[g], c.data=c.data[[g]], psi.inv=psi.inv[,g]) else base::matrix(, nr=ifelse(Q0[g], 0, nn[g]), nc=Qs[g]))
    ...
    # Local Shrinkage
      ...
      phi[Gs]        <- lapply(Gs, function(g) if(nn0[g]) sim.phi(Q=Qs[g], P=P, nu=nu, 
