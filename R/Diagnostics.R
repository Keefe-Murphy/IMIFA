...
get_IMIFA_results.IMIFA        <- function(sims = NULL, burnin = 0L, thinning = 1L, G = NULL, Q = NULL, criterion = c("bicm", "aicm", "dic", "bic.mcmc", "aic.mcmc"),
  ...
  if(Q.T)    {
    ...
    if(inf.Q)  {
      ...
      Qtmp       <- if(inf.G) sims[[1]][[1]]$Q.store[seq_len(G),, drop=FALSE] else switch(method, MIFA=sims[[if(G.T) G == n.grp else G.ind]][[1]]$Q.store, sims[[1]][[1]]$Q.store)
  ...
  if(clust.ind) {
    ...
    if(!label.miss) {
      ...
      if(length(levs) == G) {
        ...
        z.tmp    <- lapply(seq_along(tmp.store), function(i) factor(factor(z[i,], labels=left[tabulate(z[i,], nbins=G) > 0]), levels=right))
...
# Retrieve (unrotated) scores
  ...
  if(inf.Q) {
    ts           <- seq_along(tmp.store)
    Lstore       <- lapply(Gseq, function(g) ts[Q.store[g,] >= Q[g]])
