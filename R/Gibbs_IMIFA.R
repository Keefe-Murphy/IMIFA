    ...
    # Cluster Labels
      ...
      if(G > 1)     {
        ...
        if(uni) {
          ...
          log.check  <- utils::capture.output(log.probs <- try(vapply(Gs, function(g, Q=Q0[g]) dmvn(data, mu[,g], if(Q) sigma[[g]] else sq_mat(sigma[[g]]), log=TRUE, isChol=!Q), numeric(N)), silent=TRUE))
        }
