...
#' @param plot.meth The type of plot to be produced for the \code{param} of interest, where \code{correlation} refers to ACF/PACF plots, \code{means} refers to posterior means, \code{density}, \code{trace} and \code{parallel.coords} are self-explanatory. "\code{all}" in this case, the default, refers to {\code{trace, density, means, correlation}}. \code{parallel.coords} is only available when \code{param} is one of \code{means}, \code{loadings} or \code{uniquenesses}. Special types of plots which don't require a \code{param} are \code{GQ}, for plotting the posterior summaries of the numbers of groups/factors, if available, \code{zlabels} for plotting clustering uncertainties if clustering has taken place, and \code{errors} for visualing the difference between the estimated and empirical covariance matrix/matrices.
...
#' @param palette An optional colour palette to be supplied if overwriting the default palette set inside the function is desired.
...
#' @param by.fac When \code{mat} is TRUE (the default), by.fac optionally allows (mat)plotting of scores and loadings by factor (for a given observation or variable, respetively, controlled by \code{ind}) when set to TRUE (the default). Otherwise every observation or variable is plotted for a given factor, controlled by \code{ind}.
...
plot.Results_IMIFA  <- function(x = NULL, plot.meth = c("all", "correlation", "density", "errors", "GQ", "means", "parallel.coords", "trace", "zlabels"),
  ...
  indx    <- missing(ind)
  facx    <- all(missing(fac), is.element(param, c("scores", "loadings")))
  gx      <- missing(g)
  if(!indx) {
    ind   <- as.integer(ind)
    xind  <- ind
