...
plot.Results_IMIFA  <- function(x, plot.meth = c("all", "correlation", "density", "errors", "GQ", "means", "parallel.coords", "trace", "zlabels"), param = c("means", "scores", "loadings", "uniquenesses", "pis", "alpha", "discount"), g = NULL,
  ...
  for(g in Gs) {
    ...
    if(m.sw["M.sw"])  {
      ...
      if(param == "scores") {
        labs   <- if(!grp.ind)  1L else if(show.last) clust$last.z else clust$MAP
        ...
        if(isTRUE(heat.map))    {
          ...
          if(mispal && G >= 2) grDevices::palette(viridis(max(G, 2L), alpha=transparency))
    ...
    if(m.sw["E.sw"]) {
     ...
     if(Hind) {
       ...
       dat.x[dat.x == 0]   <- NA
       suppressWarnings(.plot_CI(graphics::barplot(dat.x, ylim=c(0L, max(ci.x[3L,], dat.x, na.rm=TRUE)), col=grey),
                        ci.x[2L,], li=ci.x[1L,], ui=ci.x[3L,], add=TRUE, gap=TRUE, slty=3, scol="red", pch=15))
       if(titles) {
...
#' heat_legend(data, cols); box(lwd=2)
  heat_legend  <- function(data, cols, breaks = NULL, cex.lab = 1, ...) {
...
#' # (PY <- G_priorDensity(N=50, alpha=c(19.23356, 6.47006, 1), discount=c(0, 0.47002, 0.7300045)))
  G_priorDensity      <- function(N, alpha, discount = 0, show.plot = TRUE) {
    ...
    if(isTRUE(show.plot))   {
      cols     <- seq(from=2, to=max.len + 1L)
      grDevices::palette(grDevices::adjustcolor(rep(cols, 2L), alpha.f=ifelse(grDevices::dev.capabilities()$semiTransparency, 0.5, 1)))
