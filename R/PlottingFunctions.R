...
#' @param plot.meth The type of plot to be produced for the \code{param} of interest, where \code{correlation} refers to ACF/PACF plots, \code{means} refers to posterior means, \code{density}, \code{trace} and \code{parallel.coords} are self-explanatory. "\code{all}" in this case, the default, refers to {\code{trace, density, means, correlation}}. Special types of plots which don't require a \code{param} are \code{GQ}, for plotting the posterior summaries of the numbers of groups/factors, if available, \code{zlabels} for plotting clustering uncertainties if clustering has taken place, and \code{errors} for visualing the difference between the estimated and empirical covariance matrix/matrices.
...
#' # plot(resIMIFA, "all", "uniquenesses", g=1, partial=TRUE)
#' # plot(resIMIFA, "all", "pis", intervals=FALSE)
