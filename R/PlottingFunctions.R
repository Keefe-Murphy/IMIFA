...
plot.Results_IMIFA  <- function(x = NULL, plot.meth = c("all", "correlation", "density", "errors", "GQ", "means", "parallel.coords", "trace", "zlabels"),
  ...
  for(g in Gs) {
    ...
    if(m.sw["E.sw"]) {
      ...
      pl.x   <- graphics::barplot(plot.x, beside=TRUE, col=col.e, main="", ylab="Error", density=dens)
      na.x   <- G > 1 & is.na(x.plot[[1]])
      sumna  <- sum(na.x)
      if(G > 1) graphics::points(x=Rfast::colMedians(pl.x[,na.x, drop=FALSE]), y=vector("numeric", sumna), pch=8, col="red", cex=1.5)
