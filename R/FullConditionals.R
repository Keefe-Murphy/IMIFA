...
# Full Conditionals
  ...
  # Cluster Labels
    ...
    .sim_z.inf   <- function(data, mu, sigma, Gseq, N, pi.prop, Q0, G, log.slice.ind = 0) {
      ...
      if(sum(ind) != 0)   {
        G3s      <- seq_len(if(sum(ind) > (G - G2)) sum(colSums(is.finite(log.slice.ind[ind,-G2s, drop=FALSE])) > 0) else max(Rfast::rowsums(is.finite(log.slice.ind[ind,-G2s, drop=FALSE]))))
        lnp      <- cbind(lnp[ind,G2, drop=FALSE], sweep(log.num[ind,Gseq[-G2s][G3s], drop=FALSE], 1, log.denom[ind], FUN="-"))
        for(g in G3s + 1) {
         lnp[,g] <- matrixStats::rowLogSumExps(lnp, cols=g:(g - 1))
        }
