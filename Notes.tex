...
\begin{flalign}
	...
	& \propto \given\Psi\given^{-\frac{1}{2}} \exp\left(-\frac{1}{2}\trace\left[\Psi^{-1}\left(X - \eta\Lambda\right)^T\left(X - \eta\Lambda\right)\right]\right)\nonumber&\\
...
\mathrm{P}\left(\Psi\right) & \propto \given\Psi^{-1}\given^{\frac{N + p + 1}{2}}\exp\left(-\frac{1}{2}\trace\left(\mathcal{S}^{-1^\star}\Psi\right)\right)&\nonumber\\
...
\mathrm{P}\left(\Psi^{-1}\right) & \propto \given\Psi^{-1}\given^{\frac{N}{2}}\exp\left(-\frac{1}{2}\trace\left(\mathcal{S}^\star\Psi^{-1}\right)\right)\nonumber
\shortintertext{Since $\Psi$ is a diagonal matrix$\colon$}
\mathrm{P}\left(\Psi^{-1}\right) & \propto \prod_{j=1}^{p}\given\psi_j^{-1}\given^{\frac{N}{2}}\exp\left(-\frac{1}{2}\trace\left(\mathcal{S}_j^\star\psi_j^{-1}\right)\right)\nonumber
\shortintertext{This suggests the prior for $\Psi^{-1}$ is a product of $p \hspace{2mm} \textrm{Ga}\left(\alpha,\beta\right)$ distributions. We choose hyperparameters as per \citet{Fruhwirth-Schnatter2010-II}, by bounding each $\psi_j$ away from zero in such a way that Heywood problems are avoided, with a sufficiently large shape $\alpha$ and variable-specific rates $\beta_j$ derived from $\frac{\alpha - 1}{\left(\mathbf{S}^{-1}\right)_{jj}}$. If $N\ll p$, however, we find it useful to constrain uniquenesses to be isotropic in order to reduce prior senstitivity, and derive the single rate hyperparameter from $\frac{p\left(\alpha - 1\right)}{\sum_{j=1}^p\left(\mathbf{S}_{jj}\right)^{-1}}$.}
\mathrm{P}\left(\Psi^{-1} \given \alpha, \beta_j \right) & = \prod_{j=1}^{p}\mathrm{P}\left(\psi_j^{-1} \given \alpha, \beta_j \right) \propto \prod_{j=1}^{p} \left(\psi_j^{-1}\right)^{\alpha - 1}\exp\left(- \beta_j\psi_j^{-1}\right)\nonumber&\\
\mathrm{P}\left(\Psi^{-1}\given X^\star, \eta, \Lambda\right) &\propto \mathrm{P}\left(X^\star \given \eta,\Lambda\right)\mathrm{P}\left(\Psi^{-1} \given \alpha,\beta_j\right) \nonumber&\\
...
\mbox{where}~\mathcal{S}_j^\star &=  \sum_{i=1}^{N}\left(x_{ij} - \underline{\Lambda}_j\underline{\eta}_i\right)^T\left(x_{ij} - \underline{\Lambda}_j\underline{\eta}_i\right)\nonumber \\
\mbox{s.t.}~\mathcal{S}_j &=  \sum_{i=1}^{N}\left(x_{ij} - \mu_j - \underline{\Lambda}_j\underline{\eta}_i\right)^T\left(x_{ij} - \mu_j - \underline{\Lambda}_j\underline{\eta}_i\right) \nonumber&
\shortintertext{Thus the posterior distribution of each $\psi_j^{-1}$ is given by:}
\mathrm{P}\left(\psi_j^{-1}\given X,F,\Lambda\right) & \sim \textrm{Ga}\left(\alpha + \frac{N}{2},\beta_j + \frac{S_j}{2}\right)\label{eq:18}\shortintertext{If uniquenesses are assumed to be isotropic, we instead have:}
\mathrm{P}\left(\psi^{-1}\given X,F,\Lambda\right) & \sim \textrm{Ga}\left(\alpha + \frac{Np}{2},\beta + \frac{\trace{\left(S\right)}}{2}\right)\label{eq:19}
\end{flalign}
...
\mbox{s.t.}\quad \underline{\lambda}_j \given \underline{\phi}_j,\underline{\tau}&\sim \textrm{MVN}_{q^\star}\left(\underline{0},\mathrm{D}_j\right)\label{eq:22}&\\
...
\delta_1 &\sim \textrm{Ga}\left(\alpha_1,\beta_1\right), \quad\delta_h \sim \textrm{Ga}\left(\alpha_2,\beta_2\right), \quad h \geq 2\label{eq:24}&
\end{flalign}
