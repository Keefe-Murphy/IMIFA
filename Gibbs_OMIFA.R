  ...
  # Iterate
    ...
    # Scores & Loadings
      ...
      if(!any(Q0))    {
        ...
        lmat         <- lapply(Gseq, function(g) base::matrix(, nr=P, nc=0))
      } else {
        eta.tmp      <- lapply(Gseq, function(g) if(all(nn0[g], Q0[g])) sim.score(N=nn[g], lmat=lmat[[g]], Q=Qs[g], Q1=Q1[g], c.data=c.data[[g]], psi.inv=psi.inv[,g]) else base::matrix(, nr=ifelse(Q0[g], 0, nn[g]), nc=Qs[g]))
