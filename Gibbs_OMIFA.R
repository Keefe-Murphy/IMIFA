...
# Gibbs Sampler Function
  ...
  # Iterate
    ...
    # Cluster Labels
      ...
      Q0             <- Qs  > 0
      Q1             <- Qs == 1
      z.log          <- capture.output({ z.res <- try(.sim.z(data=data, mu=mu, sigma=sigma, Gseq=Gseq, N=N, pi.prop=pi.prop, Q0=Q0[Gs]), silent=TRUE) })
    ...
    # Scores & Loadings
      ...
      if(!any(Q0))    {
        ...
        lmat         <- lapply(Gseq, function(g) if(all(nn0[g], Q0[g])) matrix(unlist(lapply(Pseq, function(j) .sim.load.s(Q=Qs[g], c.data=c.data[[g]][,j], Q1=Q1[g], 
